...
Your task is to provide only the exact CLI command that answers the user's question. 
Do not provide explanations, thoughts, or any extra text—just the command.
...
def ask_deepseek(prompt):
    ...
    try:
        ...
        # Clean the response to only return the command part
...
def clean_command_response(response):
    # Remove any explanatory text (e.g., "To obtain the status of...") and non-command parts
    response = re.sub(r'^(.*?use the following CLI command\s*|^To.*?$)', '', response, flags=re.DOTALL)
    
    # Remove any non-CLI command characters (bash, ios, etc.)
    response = re.sub(r'\b(bash|ios|.*?command)\b', '', response, flags=re.IGNORECASE)
    
    # Strip leading/trailing whitespaces and replace any multiple spaces/newlines with a single space
