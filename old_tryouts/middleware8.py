...
Your task is to provide the exact CLI command that answers the user's question.
Please provide the command **inside quotation marks** only. Do not include any extra text or explanations.
...
def ask_deepseek(prompt):
    ...
    try:
        ...
        # Clean the response to extract only the command within quotation marks
...
def clean_command_response(response):
    # Extract the command inside quotation marks
    match = re.search(r'"(.*?)"', response)
    if match:
        return match.group(1).strip()  # Return the command without the quotes
    else:
